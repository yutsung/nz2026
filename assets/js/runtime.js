 ...
 *   Left click / wheel-down  next slide · Right click / wheel-up  previous slide
 *     (skipped for data-format="booklet" decks — those scroll instead of paginate)
 *   Auto-injected "操作說明" button + modal explaining all controls (no markup needed)
 *   F  fullscreen
...
(function () {
  ...
  ready(function () {
    ...
    function go(n, fromRemote){
      ...
      // help button only shown on the first slide
      const helpBtnEl = document.querySelector('.help-btn');
      if (helpBtnEl) helpBtnEl.classList.toggle('is-hidden', n !== 0);
...
<div class="hint-bar">
  <span><kbd>← →</kbd> 翻頁</span>
  <span><kbd>R</kbd> 重置計時</span>
  <span><kbd>Esc</kbd> 關閉</span>
  <span style="color:#6e7681">拖曳卡片頂部移動 · 拖曳右下角調整大小</span>
  <button class="reset-layout" id="reset-layout">重置版面</button>
</div>
...
(function(){
  ...
  function update(n) {
    ...
    notesBody.innerHTML = note || '<span class="empty">（這一頁還沒有逐字稿）</span>';
  ...
  notesBody.innerHTML = slideMeta[idx].notes || '<span class="empty">（這一頁還沒有逐字稿）</span>';
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    document.addEventListener('keydown', function (e) {
      ...
      switch (e.key) {
        ...
        case 'Escape': toggleOverview(false); toggleNotes(false); toggleHelp(false); break;
      }
    });
    ...
    /* ===== Mouse + wheel navigation, plus auto-injected help button/modal =====
     *   left click            -> next slide
     *   right click           -> previous slide (browser context menu suppressed)
     *   wheel down/up         -> next/previous slide (debounced: one scroll = one page)
     * Skipped entirely for booklet format (data-format="booklet") — those decks
     * are read by scrolling/tapping, not paginated, so click-to-advance would
     * fight normal reading gestures.
     * The "操作說明" button + modal are created here (like .progress-bar /
     * .overview above) so NO per-deck markup is ever required.
     */
    const isBooklet = deck.getAttribute('data-format') === 'booklet';
    ...
    if (!isBooklet) {
      function isNavBlocked(el) {
        return !!(el && el.closest && el.closest(
          '.overview, .notes-overlay, .progress-bar, .help-modal, .help-btn, button, a[href], input, textarea, select, [contenteditable="true"]'
        ));
      }
      document.addEventListener('click', function (e) {
        if (e.button !== 0) return;
        const sel = window.getSelection();
        if (sel && sel.toString().length) return; // don't hijack text selection
        if (isNavBlocked(e.target)) return;
        go(idx + 1);
      });
      ...
      document.addEventListener('wheel', function (e) {
        ...
        setTimeout(function () { _wheelBusy = false; }, 550);
      }, { passive: true });
      ...
      if (!helpModal) {
        ...
        helpModal.innerHTML =
          '<div class="help-modal-card" role="dialog" aria-modal="true" aria-label="操作說明">' +
            '<button type="button" class="help-modal-close" aria-label="關閉操作說明">✕</button>' +
            '<h3>操作說明</h3>' +
            '<ul>' + shortcuts.map(function (s) {
              return '<li><span class="key">' + s[0] + '</span><span>' + s[1] + '</span></li>';
            }).join('') + '</ul>' +
          '</div>';
        document.body.appendChild(helpModal);
      }
      helpModal.addEventListener('click', function (e) { if (e.target === helpModal) toggleHelp(false); });
      const helpCloseBtn = helpModal.querySelector('.help-modal-close');
      if (helpCloseBtn) helpCloseBtn.addEventListener('click', function () { toggleHelp(false); });
      helpBtn.addEventListener('click', function (e) { e.stopPropagation(); toggleHelp(true); });
    }
    ...
    /* ===== Scale-to-fit with black letterbox =====
     * Scales the deck to fit any viewport while preserving the design-time
     * aspect ratio (16:9 by default, or whatever data-ref-w/h specifies).
     * Extra space is filled with black — no theme colour bleeding.
     * Skip for booklet format (data-format="booklet") which handles its own sizing.
     */
    if (deck.getAttribute('data-format') !== 'booklet') {
      const refW = parseInt(deck.getAttribute('data-ref-w') || '1920', 10);
      const refH = parseInt(deck.getAttribute('data-ref-h') || '1080', 10);
      function applyDeckScale() {
        const vw = window.innerWidth, vh = window.innerHeight;
        const scale = Math.min(vw / refW, vh / refH);
        const sw = refW * scale, sh = refH * scale;
        deck.style.width          = refW + 'px';
        deck.style.height         = refH + 'px';
        deck.style.position       = 'absolute';
        deck.style.transform      = 'scale(' + scale + ')';
        deck.style.transformOrigin = 'top left';
        deck.style.left           = Math.round((vw - sw) / 2) + 'px';
        deck.style.top            = Math.round((vh - sh) / 2) + 'px';
        document.documentElement.style.background = '#000';
        document.body.style.background = '#000';
      }
      applyDeckScale();
      window.addEventListener('resize', applyDeckScale);
    }
  });
})();
